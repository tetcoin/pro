...
fn spread_layout_struct_derive(s: &synstructure::Structure) -> TokenStream2 {
    ...
    let pull_body = variant.construct(|field, _index| {
        let ty = &field.ty;
        quote! {
            <#ty as ::pro_storage::traits::SpreadLayout>::pull_spread(__key_ptr)
        }
    });
    let push_body = variant.each(|binding| {
        quote! {
            ::pro_storage::traits::SpreadLayout::push_spread(#binding, __key_ptr);
        }
    });
    let clear_body = s.each(|field| {
        quote! {
            ::pro_storage::traits::SpreadLayout::clear_spread(#field, __key_ptr);
        }
    });
    s.gen_impl(quote! {
        gen impl ::pro_storage::traits::SpreadLayout for @Self {
...
fn spread_layout_enum_derive(s: &synstructure::Structure) -> TokenStream2 {
    ...
    let push_body = s.variants().iter().enumerate().map(|(index, variant)| {
        ...
        let fields = variant.bindings().iter().map(|field| {
            quote! {
                ::pro_storage::traits::SpreadLayout::push_spread(#field, __key_ptr);
            }
        });
        quote! {
            #pat => {
                { <u8 as ::pro_storage::traits::SpreadLayout>::push_spread(&#index, __key_ptr); }
                #(
                    { #fields }
                )*
            }
...
/// Derives `pro_storage`'s `SpreadLayout` trait for the given `struct` or `enum`.
